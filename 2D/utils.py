    ...
    if net == 'sam':
        ...
        if args.encoder not in options:
            ...
            if args.sam_ckpt is None:
                args.sam_ckpt = 'sam_vit_b_01ec64.pth'  # provide default checkpoint
    ...
    if c == 2: # for REFUGE multi mask output
        ...
        if points != None:
            for i in range(b):
                ...
                # ps = ps[..., :2]
                # print(f"Debug: Adjusted ps shape: {ps.shape}")
                # print(f"Debug: gt_masks shape: {gt_masks.shape}, points shape: {points.shape}")
                # for i in range(len(ps)):
                #     for point in ps[i]:
                #         x, y = point[0].item(), point[1].item()
                #         print(f"Debug: Batch {i}, Point ({x}, {y})")
                #         if 5 <= x < gt_masks.shape[2] - 5 and 5 <= y < gt_masks.shape[3] - 5:
                #             gt_masks[i, 0, x-5:x+5, y-5:y+5] = 0.5
                #             gt_masks[i, 1, x-5:x+5, y-5:y+5] = 0.1
                #             gt_masks[i, 2, x-5:x+5, y-5:y+5] = 0.4
                #         else:
...
def vis_image_color(imgs, pred_masks, gt_masks, save_path, points=None, class_colors=None):
    """
    Visualize input images, predictions, and ground truth in color.
    ...
    """
    b, c, h, w = pred_masks.size()
    row_num = min(b, 4)  # Show at most 4 samples in one row
    ...
    for i in range(row_num):
        ...
        for cls in range(c):
            pred_mask = (pred_masks[i, cls, :, :] > 0.5).cpu().numpy()  # Binarize predictions
            gt_mask = (gt_masks[i, cls, :, :] > 0.5).cpu().numpy()      # Binarize ground truth

            # Apply colors for the class
            pred_rgb[pred_mask] = class_colors[cls]
            gt_rgb[gt_mask] = class_colors[cls]

        # Convert input image to range [0, 1] for visualization
