...
def train_sam(args, net: nn.Module, optimizer, train_loader,
    ...
    with tqdm(total=len(train_loader), desc=f'Epoch {epoch}', unit='img') as pbar:
        for pack in train_loader:
            ...
            if args.thd:
                # print(f"masks shape before: {masks.size()}")
                imgs, pt, masks = generate_click_prompt(imgs, masks)
                # print(f"masks shape after: {masks.size()}") 
            ...
            # 打印 pred 和 masks 的形状
            # print(f"Pred shape: {pred.shape}")  # 打印预测结果的形状
            # print(f"Masks shape: {masks.shape}")  # 打印真实标签的形状
...
def validation_sam(args, val_loader, epoch, net: nn.Module, clean_dir=True):
     # eval mode
    net.eval()
    mix_res = None  # 用于累加 IOU 和 Dice 结果
    mask_type = torch.float32
    ...
    mix_res = np.array(mix_res, dtype=np.float32)  # 转换为 NumPy 数组
    mix_res /= n_val  # 进行除法运算

    num_classes = len(mix_res) // 2
    iou_per_class = mix_res[:num_classes].tolist()  # 转回 list 以便后续使用
    dice_per_class = mix_res[num_classes:].tolist()  # 同上

    avg_iou = np.mean(iou_per_class)
    avg_dice = np.mean(dice_per_class)
    # 确保 tot 变量正确
    tot = tot / n_val if isinstance(tot, (int, float)) else 0.0
