...
#     print(f"pred_masks size: {pred_masks.size()}")
#     print(f"gt_masks size: {gt_masks.size()}")
...
def vis_image(imgs, pred_masks, gt_masks, save_path, reverse = False, points=None, class_colors=None):
    """
    Visualize input images, predictions, and ground truth in color using argmax to avoid black pixels.

    Args:
        imgs: Input images, shape [B, C, H, W].
        pred_masks: Predicted masks, shape [B, C, H, W].
        gt_masks: Ground truth masks, shape [B, C, H, W].
        save_path: Path to save the visualized image.
        points: Optional points to overlay.
        class_colors: Optional list of colors for each class. Default is None.
    """
    
    b, c, h, w = pred_masks.size()
    # Calculate the interval to select 4 images evenly from the batch
    interval = b // 4
    selected_indices = [i * interval for i in range(4)]
    ...
    for i, idx in enumerate(selected_indices):
        ...
        pred_classes = pred_masks[idx].argmax(dim=0).cpu().numpy()  # [H, W]
        gt_classes = gt_masks[idx].argmax(dim=0).cpu().numpy()      # [H, W]

        # Map class indices to RGB colors
        for cls in range(c):
            pred_rgb[pred_classes == cls] = class_colors[cls]
            gt_rgb[gt_classes == cls] = class_colors[cls]

        # Normalize input image to range [0, 1] for visualization
        img = imgs[idx].permute(1, 2, 0).cpu().numpy()
        img = (img - img.min()) / (img.max() - img.min())

        # Plot the input image, predictions, and ground truth
        fig, axes = plt.subplots(1, 3, figsize=(15, 5))
        axes[0].imshow(img)
        axes[0].set_title("Input Image")
        axes[0].axis("off")
        axes[1].imshow(pred_rgb)
        axes[1].set_title("Prediction (Color)")
        axes[1].axis("off")

        axes[2].imshow(gt_rgb)
        axes[2].set_title("Ground Truth (Color)")
        axes[2].axis("off")

        # Save the figure
        plt.tight_layout()
        plt.savefig(f"{save_path}_sample_{i}.png")
        plt.close()
...
def eval_seg(pred, true_mask_p, threshold):
    """
    threshold: an int or a tuple of ints
    masks: [b,2,h,w]
    pred: [b,2,h,w]
    """
    # Ensure `pred` is a numpy.ndarray
    if isinstance(pred, torch.Tensor):
        pred = pred.cpu().numpy()
    if isinstance(true_mask_p, torch.Tensor):
    ...
    if c == 2:
        iou_d, iou_c, disc_dice, cup_dice = 0, 0, 0, 0
        for th in threshold:
            gt_vmask_p = (true_mask_p > th).astype(np.float32)
            vpred = (pred > th).astype(np.float32)

            disc_pred = vpred[:, 0, :, :].astype("int32")
            cup_pred = vpred[:, 1, :, :].astype("int32")

            disc_mask = gt_vmask_p[:, 0, :, :].astype("int32")
            cup_mask = gt_vmask_p[:, 1, :, :].astype("int32")
            ...
            # Dice for numpy
            disc_dice += dice_coeff_numpy(vpred[:, 0, :, :], gt_vmask_p[:, 0, :, :])
            cup_dice += dice_coeff_numpy(vpred[:, 1, :, :], gt_vmask_p[:, 1, :, :])
        ...
        for th in threshold:
            ...
            for i in range(c):
                pred_i = vpred[:, i, :, :].astype("int32")
                mask_i = gt_vmask_p[:, i, :, :].astype("int32")

                # IOU for numpy
                ious[i] += iou(pred_i, mask_i)

                # Dice for numpy
                dices[i] += dice_coeff_numpy(vpred[:, i, :, :], gt_vmask_p[:, i, :, :])
        ...
        for th in threshold:
            ...
            # Dice for numpy
            edice += dice_coeff_numpy(vpred[:, 0, :, :], gt_vmask_p[:, 0, :, :])
...
def generate_click_prompt(img, msk0, pt_label = 1):
    # return: prompt, prompt mask
    msk=msk0
    pt_list = []
    msk_list = []
    # print(f"msk shape: {msk0.size()}") 
    b, c, h, w, d = msk.size()
    msk = msk[:,0,:,:,:]
    for i in range(d):
        pt_list_s = []
    ...
    msk = msk.unsqueeze(1)
    # print(f"msk shape : {msk.size()}") 
    # print(f"pt shape before permute: {pt.shape}")
    
    return img, pt, msk0 #[b, 2, d], [b, c, h, w, d]

# def generate_click_prompt(img, msk, pt_label=1):
#     # return: prompt, prompt mask
#     pt_list = []
#     msk_list = []
#     print(f"msk shape before: {msk.size()}") 
#     b, c, h, w, d = msk.size()
...
#         msk_list_s = []
#         for j in range(b):
#             for ch in range(c):  # Loop through each channel
#                 msk_s = msk[j, ch, :, :, i]
#                 indices = torch.nonzero(msk_s)
#                 if indices.size(0) == 0:
...
#         pts = torch.stack(pt_list_s, dim=0).view(b, c, -1)  # Preserve batch and channel
#         msks = torch.stack(msk_list_s, dim=0).view(b, c, h, w)
...
#     print(f"msk shape after: {msk.size()}") 
#     print(f"pt shape before permute: {pt.shape}")
#     pt = pt.permute(0, 2, 1, 3)  # 重新排列维度
#     pt = pt.reshape(pt.shape[0] * pt.shape[1], pt.shape[2], pt.shape[3])  # 合并前两个维度
#     print(f"pt shape after reshape: {pt.shape}")
#     return img, pt, msk  # Return [b, c, d], [b, c, h, w, d]
